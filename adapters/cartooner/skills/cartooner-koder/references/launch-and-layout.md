...
1. tool/runtime change
   - `python3 {CLAWSEAT_ROOT}/.agent/scripts/agent_admin.py session switch-harness <seat> <tool> <mode> <provider> --project cartooner`
2. auth/provider-only change on the same tool
   - `python3 {CLAWSEAT_ROOT}/.agent/scripts/agent_admin.py engineer rebind <seat> --project cartooner <mode> <provider>`
...
- and, when durable proof is needed, stamp
  `python3 {CLAWSEAT_ROOT}/.agents/skills/gstack-harness/scripts/ack_contract.py --profile {CLAWSEAT_ROOT}/adapters/cartooner/profiles/cartooner.toml --seat <seat>`
