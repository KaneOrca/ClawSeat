...
When you must use `<repo-root>/.scripts/send-and-verify.sh` directly in a
...
```bash
python3 <repo-root>/.agents/skills/gstack-harness/scripts/dispatch_task.py \
...
```bash
python3 <repo-root>/.agents/skills/gstack-harness/scripts/complete_handoff.py \
  --profile <project-profile.toml> \
  --source engineer-a \
  --target engineer-b \
  --task-id <TASK_ID> \
