...
def test_load_shared_secrets_loads_env_files(tmp_path):
    ...
    env = _bash_invoke_load_shared_secrets(
        tmp_path,
        {
            "minimax.env": 'MINIMAX_API_KEY="minimax-test-value-1"\n',
            "gemini.env": 'GEMINI_API_KEY="gemini-test-value-2"\n',
            "xcode-best.env": 'XCODE_BEST_GPT_IMAGE_API_KEY="xcb-test-value-3"\n',
        },
    )
...
def test_load_shared_secrets_default_path_is_real_home_agents(tmp_path):
    ...
    (default_dir / "default.env").write_text(
        'DEFAULT_PATH_KEY="from-real-home-default"\n', encoding="utf-8"
    )
    script = f"""
set -euo pipefail
...
"""
    ...
    assert result.returncode == 0, result.stderr
    assert "DEFAULT_PATH_KEY=from-real-home-default" in result.stdout
