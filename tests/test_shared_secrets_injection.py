...
def test_shared_secrets_dir_injected_for_codex_oauth(
    ...
    (shared_dir / "minimax.env").write_text("MINIMAX_API_KEY=test-mm-key-XYZ\n", encoding="utf-8")
...
def test_auth_tied_secret_wins_over_shared(
    ...
    shared_dir = fake_home / ".agents" / "secrets" / "shared"
    shared_dir.mkdir(parents=True)
    (shared_dir / "minimax.env").write_text("MINIMAX_API_KEY=shared-value\n", encoding="utf-8")
...
def test_explicit_shared_secrets_dir_env_override(
    ...
    (custom_dir / "x.env").write_text("MY_KEY=via-explicit\n", encoding="utf-8")
