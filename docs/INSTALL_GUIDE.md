...
agent 会按 6-phase 流程继续：
P0 Preflight + Credentials + Bootstrap → P1 Memory online + system-scan →
P2 Query memory → operator picks target agent →
P3 Overlay + `/new` identity + Feishu group creation →
P4 Configure + start backend seats + Feishu smoke → P5 Handoff。
详见 `core/skills/clawseat-install/references/install-flow.md`。
...
# agent 会自动执行 6 阶段流程：
#   P0 preflight + credentials + bootstrap
#   P1 memory seat online + system-scan
#   P2 query memory → 选择目标 agent
...
- 本地 Claude/Codex `/cs`: `examples/starter/profiles/install-with-memory.toml`
...
| `examples/starter/profiles/install-with-memory.toml` | canonical 本地 `/cs` 安装项目（memory + koder + planner + builder + reviewer） |
| `examples/starter/profiles/install.toml` | legacy 本地 memory-less 变体 |
