...
if [[ -z "$EXEC_MODE" ]]; then
  ...
  if [[ "$SKIP_ANCESTOR_PREFLIGHT" != "1" && "$SESSION_NAME" =~ ^(.+)-ancestor-(claude|codex|gemini)$ ]]; then
    ...
    if [[ -f "$_profile_path" ]]; then
      ...
      echo "ancestor-preflight: reusing existing profile for '$_preflight_project'"
    else
    ...
    unset _preflight_project _preflight_tool _profile_path _brief_path _binding_path _clawseat_root _clawseat_core _wizard_args
  fi
  # ────────────────────────────────────────────────────────────────────
