...
10. 六宫格丢失时，优先 `agent_admin window open-grid ${PROJECT_NAME} [--recover] [--open-memory]`；它按 project seat roster + `wait-for-seat.sh` prefix 重新生成 payload，`--recover` 只 focus 已存在窗口，不重开。
...
2. 除 ancestor 外，每个 pane 都运行 `wait-for-seat.sh <project-seat>`：轮询 tmux session，seat 一出现就 attach；seat 重启或 client 断开后会自动 re-attach 回同一 iTerm pane。
...
- `wait-for-seat.sh` 会把重启后的 seat 自动 re-attach 回原来的 iTerm pane；不要手动 `tmux attach` 抢这些 pane
