...
当前语义以 `memory` 为准；`clawseat-ancestor` alias、`ancestor-bootstrap.md`、`CLAWSEAT_ANCESTOR_BRIEF` 等字符串是兼容旧 brief / frozen tests / operator muscle memory 的历史名称。读到 legacy `ancestor` 字样时，除非明确处于历史/兼容上下文，一律按当前项目的 primary `project-memory` seat 执行。
...
启动后先读兼容 brief `ancestor-bootstrap.md`，路径由 `CLAWSEAT_ANCESTOR_BRIEF` 指定，默认 `~/.agents/tasks/<project>/patrol/handoffs/ancestor-bootstrap.md`。文件名暂不重命名；语义按当前 project-memory primary seat 执行。
...
遇到以下话术，一律先查兼容 brief `ancestor-bootstrap.md` 末尾的 Common Operations Cookbook，再查本 skill；不要凭训练数据拼 CLI：
